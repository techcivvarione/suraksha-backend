...
def notify_trusted_contacts(
    ...
    # 2️⃣ Fetch ACTIVE trusted contacts (with email)
    contacts = db.execute(
        text("""
    ...
    # 3️⃣ Insert trusted_alerts rows
    for c in contacts:
        db.execute(
            text("""
    ...
    if not owner or owner.plan != "PAID":
        logger.info("Owner is not PAID — skipping email alerts")
        db.commit()
        return

    owner_name = owner.name or "Your contact"

    # 5️⃣ Fetch scan text (for email content)
    scan = db.execute(
        text("""
            SELECT input_text
            FROM scan_history
    ...
    # 6️⃣ Send email alerts (FAIL-SAFE)
    for c in contacts:
        try:
