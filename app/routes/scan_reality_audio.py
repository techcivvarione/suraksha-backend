...
async def scan_reality_audio(
    ...
    try:
        ...
        try:
            detection = await audio_detector.detect(path, mime, filename=file.filename, fast_mode=fast_mode)
        ...
        signals = detection.get("signals") or [f"Voice synthesis probability {probability:.2f}"]
        risk = {
